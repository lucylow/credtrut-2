 ...
 export function RiskScoreHero() {
   ...
   useEffect(() => {
     const loadAnalytics = async () => {
       try {
         const data = await fetchCreditAnalytics();
         setAnalytics(data);
       } catch (error) {
   ...
   return (
     ...
     >
       <div className={`glass-card p-8 bg-gradient-to-br ${colors.bg}`}>
         ...
         {analytics && (
           <div className="mt-8 pt-6 border-t border-primary/10 grid grid-cols-2 lg:grid-cols-4 gap-4">
             <div className="space-y-1">
               <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">Sharpe Ratio</p>
               <p className="text-lg font-bold text-foreground">{analytics.sharpeRatio}</p>
             </div>
             <div className="space-y-1">
               <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">Volatility</p>
               <p className="text-lg font-bold text-foreground">{analytics.volatilityScore}%</p>
             </div>
             <div className="space-y-1">
               <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">Default Prob.</p>
               <p className="text-lg font-bold text-foreground">{analytics.defaultProbability}%</p>
             </div>
             <div className="space-y-1">
               <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">Market Sentiment</p>
               <div className="flex items-center gap-1.5">
                 <div className="w-2 h-2 rounded-full bg-emerald-500" />
                 <p className="text-lg font-bold text-foreground">Bullish</p>
               </div>
             </div>
           </div>
         )}
